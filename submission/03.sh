# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

# get the redeem script from the transaction
redeem_script=$(bitcoin-cli -regtest -named decoderawtransaction hexstring=$transaction | jq -r '.vin[0].scriptSig.asm | split(" ")| .[-1]' )


# get the P2SH address from the redeem script
P2SH_Segwit=$(bitcoin-cli -regtest decodescript $redeem_script | jq -r '.segwit | ."p2sh-segwit"')

echo $P2SH_Segwit